...
import { 
  ...
  BellOutlined, MoonOutlined, SunOutlined
...
const decodeToken = () => {
  ...
  if (token) {
    try {
      ...
      const rol = decoded.role || decoded.Rol || 'Admin';
      const basHarfler = adSoyad.charAt(0).toUpperCase();
...
const App = () => {
  ...
  useEffect(() => {
    setCollapsed(screens.xs || screens.sm);
  }, [screens]);
  ...
  const getMenuItems = () => {
    const isSekmeAdmin = kullanici.rol.toLowerCase() === 'admin';
    const items = [
  ...
  return (
    ...
    >
      ...
      <Layout style={{ minHeight: '100vh' }}>
        ...
        <Sider trigger={null} collapsible collapsed={collapsed} breakpoint="lg" collapsedWidth={screens.xs ? 0 : 80} width={240} style={{ background: appTheme.sidebar, borderRight: isDarkMode ? `1px solid ${appTheme.border}` : 'none', height: 'calc(100vh - 32px)', position: 'fixed', left: 16, top: 16, bottom: 16, zIndex: 100, borderRadius: 24, overflow: 'hidden' }}>
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <div style={{ height: 60, display: 'flex', alignItems: 'center', justifyContent: 'center' }}></div>
        ...
        <Layout style={{ marginLeft: screens.xs ? 0 : (collapsed ? 112 : 272), transition: 'margin-left 0.2s', minHeight: '100vh', paddingRight: 16 }}>
          <Header style={{ padding: screens.xs ? '0 12px' : '0 24px', background: 'transparent', display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 100, zIndex: 97 }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <Button type="text" icon={collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />} onClick={() => setCollapsed(!collapsed)} style={{ fontSize: '18px', width: 48, height: 48, color: appTheme.text, marginRight: screens.xs ? 0 : 16 }} />
              {!screens.xs && <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, color: appTheme.text }}>Stok Takip</h1>}
            </div>
            ...
            <Space size={16} style={{ background: appTheme.headerControlBg, padding: '8px 16px', borderRadius: 40, border: `1px solid ${appTheme.border}`, boxShadow: '0 2px 10px rgba(0,0,0,0.03)' }}>
              {isDarkMode ? <SunOutlined style={{ fontSize: 18, color: appTheme.text, cursor: 'pointer' }} onClick={toggleTheme} /> : <MoonOutlined style={{ fontSize: 18, color: appTheme.text, cursor: 'pointer' }} onClick={toggleTheme} />}
